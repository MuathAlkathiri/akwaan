...
Parent Catalog: `gaming/SKILL.md`  
...
Treat title, era/subseries, gameplay mode, Question Pattern, and Gameplay
...
- Easy / 200: familiar-but-not-instant items from player memory — widely
  remembered maps, weapons, Multiplayer experiences, and distinctive sounds.
  `Raid` and `Firing Range` are model *concepts*, not a repeat quota, and only
  remain Easy when the frame is not instantly obvious.
- Medium / 400: regular-player weapons such as `UMP-45` or `Galil`, perks,
  equipment, less immediate maps, HUD/UI details, and major Campaign/Zombies
  moments.
- Hard / 600: fair dedicated-player streak/audio/title identification, rare but
  distinctive UI detail, or strong secondary-mode recall. `Harrier Strike`
  from distinctive audio and `Black Ops 2` from a title-specific sound are
  models.

Hard must not come from poor capture quality, ambiguity, tiny screenshots,
obscure production facts, generic sounds, or near-identical variants.

Recalibration (2026-08-01 review): the most famous maps and weapons direct-ID
as instantly recognizable and are not fair Easy or Medium answers. `Rust`,
`Nuketown`, and `Shipment` were rejected as Recognition-Saturated. Apply the
Recognition Saturation check before labeling difficulty, and correct any
mislabeled reviewed question with `DIFFICULTY_MISCLASSIFIED` (e.g., the Zombies
Mystery Box sound plays Easy, not Medium).
...
content presented as canonical, bad aliases, quality-based difficulty,
