  ...
  implements MatchChallengeLauncher, OnModuleInit
{
  ...
  presentedContentItemIds(input: {
    ...
    // question has not been shown to anyone, so it is not spent — an abort before
    // activation returns it. Keyed off the plugin's own opt-in declaration, not a
    // slug check.
    if (
      COMBO_GAMEPLAY_PLUGIN.deadline?.source === 'runtime-state' &&
      COMBO_GAMEPLAY_PLUGIN.deadline.requiresPresentationActivation &&
