...
Operational reminders seeded from explicit Lammah rules. This file does not
...
- `DIFFICULTY_MISCLASSIFIED`: when a reviewed question plays far easier or
  harder than its label, correct the difficulty in the record and recalibrate
  the guidance rather than keeping a misleading label.
...
- `RECOGNITION_SATURATION` (call-of-duty): the CoD community has always-on
  ...
  (`QUESTION_TOO_OBVIOUS`, `UNFAIR_DIFFICULTY`). Do not use famous
  top-recognition direct IDs as Easy answers, and cap the number of famous
...
- `FAMILIAR_BUT_NOT_IMMEDIATE` (subject scope — Naruto): Prefer recognizable
  ...
  feel familiar and satisfying once revealed. For easy questions: avoid answers
  ...
  difficulty validation or cause repetition. Source: 2 approval records
  (2026-07-31: Q3 easy knowledge, Q8 hard object).
