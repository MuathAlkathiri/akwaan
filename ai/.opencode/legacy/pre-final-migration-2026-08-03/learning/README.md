...
validated, repaired, or unrejected ContentItem is not implicitly approved.

The existing history JSON is legacy evidence. Its old `Catalog`, `Subject`,
`question`, and `difficulty` fields may be read for context but must never be
copied into a new ContentItem or treated as the current DTO.
...
For new feedback, record available fields only: date, World, Scope,
ContentItem ID, prompt, structured answer payload, Content Pattern,
compatible ChallengeType IDs, Media type, Asset path/source, reason code,
original user explanation, normalized failure signature, entity, event,
feedback scope, suggested correction, future action, and status.
...
`RECOGNITION_SATURATION`, `ACCESSIBILITY_MISMATCH`, `OTHER`.
...
Demand and saturation: `RECOGNITION_SATURATION` = an answer nearly any casual
audience member recognizes instantly from the Media alone, making it trivially
obvious; `ACCESSIBILITY_MISMATCH` = the ContentItem does not suit its intended
audience or the ChallengeType's fixed pacing. Do not store a difficulty label.
...
- date, World, Scope, ContentItem ID, prompt, answer payload, Content Pattern,
  and compatible ChallengeType IDs;
...
A success signature may combine Scope, Pattern, scene/event type, answer mode,
Media type, observation, source type, audience demand, and prompt structure.
...
Match structured Subject, answer, event, Content Pattern, Asset, observation,
