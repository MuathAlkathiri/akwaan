...
## Validation Rules
Validate canonical IDs; exact native objects; a non-empty Arabic public prompt; three unique A/B/C segments with distinct content; `[2,3]`; at least one exhaustive non-overlapping 2+1 merge with all three canonical partitions safe; `mechanicPayload.candidateSets` with every segment set at least two candidates, every pair intersection at least two, and the triple intersection exactly the ground truth; safety confirmation when status is `ready`; supported inner mode; deterministic truth; no truth in mechanic data; no runtime-owned fields; no public/private leakage; no string-truth workaround; Scope evidence; and three distinct items per Challenge.
